...
function parseOpsConfig (env = process.env) {
  ...
  if (RETIRED_MTLS_SETTINGS.some(name => nonblank(env[name]))) {
    throw new Error('OPS_CONSOLE_MTLS_* settings are retired and unsupported')
  }
  ...
  if (nonblank(env.OPS_CONSOLE_TRUSTED_PROXY)) {
    throw new Error('OPS_CONSOLE_TRUSTED_PROXY is not supported')
  }

  if (originURL.protocol === 'http:') {
    if (!allowHttpLoopback || !isLoopbackHost(originURL.hostname)) {
      throw new Error('HTTP operations console requires loopback origin opt-in')
    }
